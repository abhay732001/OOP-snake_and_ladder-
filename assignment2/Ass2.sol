...
// You can edit this contract as much as you want. A template is provided here and you can change the function names and implement anything else you want, but the basic tasks mentioned here should be accomplished.
    ...
    function viewDues(address creditor) private view returns(uint256){
        return loans[creditor];
        
    }
    function settleDues(address creditor) private returns(bool){
        uint256 money=viewDues(creditor);
        if(loans[creditor]>0){
            sendCoin(creditor,money,Owner);
        }
        
        if(sendCoin(creditor,money,Owner))
            loans[creditor]=0;
    }
    // implement viewDues and settleDues which allow *ONLY* the owner
//         to *view* and *settle* his loans respectively. They take in the 
//         address of a creditor as arguments. viewDues returns a uint256 
// 	corresponding to the due amount, and does not modify any state 
// 	variables. settleDues returns a bool, true if the dues were settled 
// 	and false otherwise. Remember to set the the pending loan to 0 after 
// 	settling the dues.
    // use sendCoin function of MetaCoin contract to send the coins 
       //required for settling the dues.
